...
' . print_r( 
	...
	public function asset_needs_purchase( $asset, $asset_type, $args = array() ) {
		 ...
		 * have been resized into several different files / thumbnails, and we need to check for all of
		 * them.
		 */
		...
		if ( $asset_type == 'image' && ! empty( $array_file_names_to_query ) ) {
			...
			", '%' . $wpdb->esc_like( $file_name_to_query ) . '%' ) );
				...
				// Does the header image have "/cropped-" in it? If so, remove it so we can get a match.
