...
class Boldgrid_Inspirations_Built {
	...
	private $nav_steps = array (
		...
		'inspired' => array (
			'step-1' => array (
				'title' => 'Pages',
				'content' => 'includes/add-pages-to-existing.php' 
			),
			'step-2' => array (
				'title' => 'Themes',
	 ...
	 * Adding the meta box
	 */
	public function select_pages_meta_box() {
		$subcategories = array ();
		
		$configs = $this->inspiration->get_configs();
		
		$result = Boldgrid_Inspirations::boldgrid_api_call( 
			$configs['ajax_calls']['get_categories'], true, 
			array (
				'inspirations_mode' => $this->mode_data['mode'] 
			) );
		
		if ( isset( $result['result']['data']['categories'] ) ) {
		...
		include BOLDGRID_BASE_DIR . '/pages/includes/boldgrid-pages-meta-box.php';
	}
	...
	/**
	 * Create the accordion that is used to show the list of pages on the users site
	 *
	 * @return string
	 */
	public function create_accordion() {
		global $wp_meta_boxes;
		
		// Make a back up
		$wp_meta_boxes_copy = $wp_meta_boxes;
		
		add_meta_box( 'boldgrid-add-pages', 'BoldGrid Pages', 
			array (
				$this,
				'select_pages_meta_box' 
			), 'toplevel_page_boldgrid-inspirations', 'side', 'default' );
		
		$current_screen = get_current_screen();
		
		ob_start();
		
		do_accordion_sections( $current_screen, 'side', false );
		
		$accordion = ob_get_contents();
		
		ob_end_clean();
		
		// Reset Meta boxes, we dont want the user to be able to disable the select pages meta box
		$wp_meta_boxes = $wp_meta_boxes_copy;
	...
	public static function add_sub_menu_items( $menu_items, $top_level ) {
		...
		// Add the Pages submenu if needed
		if ( false !== array_search( 'Pages', $menu_items ) ) {
			$slug = self::get_menu_slug( $top_level );
			
			add_submenu_page( $static_top_level, 'Add New Pages', 'Add New Pages', 
				'manage_options', 
				$slug ? $slug : 'admin.php?page=boldgrid-inspirations&boldgrid-tab=pages' );
			
			if ( ! empty( $slug ) ) {
				global $submenu;
				
				$submenu['boldgrid-inspirations'][0] = array (
					'Add New Pages',
					'manage_options',
					'admin.php?page=boldgrid-inspirations&boldgrid-tab=pages',
					'Add New Pages' 
				);
			}
		}
	...
	public function inspiration_page() {
		...
		if ( isset( $_POST['task'] ) && 'deploy' == $_POST['task'] ) {
			...
			// The inspiration phase will trigger here
			if ( 'inspired' == $this->mode_data['mode'] ) {
				$accordion = $this->create_accordion();
			}
			
			$tab = isset( $_GET['boldgrid-tab'] ) ? $_GET['boldgrid-tab'] : false;
			...
			$oneliner = 'boldgrid_inspirations_type = "' . $this->mode_data['mode'] . '";';
	...
	public function generate_scenarios() {
		...
		// This is a fail safe if installing a web-site does not complete
		...
		// We are going into install, inspired setting don't matter
		if ( 'standard' == $mode ) {
