...
IMHWPB.PagenowPost = function($) {
	...
	this.gridblock_sets_append_button = function() {
		self.baseAdmin.$wrap.find('h1 a').after(
				'<a href="' + IMHWPB.gridblock_sets_admin
						+ '" class="page-title-action">' + self.baseAdmin.strings.add_gridblock_set + '</a>');
	}
	...
	this.init_gridblock_sets = function() {
		...
		if (0 == IMHWPB.configs.settings.boldgrid_menu_option) {
			...
			self.baseAdmin.$wrap_header.find('a').attr('href',
